...
public class Grid extends Actor {
  ...
  public void DrawTileAtPos(int xPos, int yPos) {
    ...
    if (getTileAtPosition(xPos, yPos) != null) {
      ...
      _processing.square(xIndex + marginX, yIndex + marginY, tileLength);
    }
  }
  @Override
  ...
  private void DrawPath() {
    ...
    if (pathArray != null && pathArray.length > 1){
      for (int i = 0; i < pathArray.length - 1; i++){
        ...
        _processing.line(pathArray[i].x * tileLength + marginX + (float)tileLength/2, 
                         pathArray[i].y * tileLength + marginY + (float)tileLength/2, 
                         pathArray[i + 1].x * tileLength + marginX + (float)tileLength/2, 
                         pathArray[i + 1].y * tileLength + marginY + (float)tileLength/2);
      }
    }
  }
